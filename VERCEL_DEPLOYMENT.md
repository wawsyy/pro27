...
2. **查看部署列表**
   - 在项目主页，你会看到所有部署
   - 最新部署会显示 **"Latest"** 标签或绿色指示器
   - 查看每个部署的时间戳

3. **识别部署类型**
   - 🌐 **生产域名**：`clothing.vercel.app`（地球图标，始终指向最新生产部署）
   - 🌿 **分支部署**：`clothing-git-main-xxx.vercel.app`（分支名，指向该分支最新部署）
   - 🔗 **预览部署**：`clothing-xxx-yyy.vercel.app`（哈希值，特定提交的预览）

4. **验证最新部署**
   - 查看最新部署的 **Commit** 信息
   - 确认 Commit 哈希是否匹配最新的 Git 提交
   - 点击部署卡片查看详情
